---
title: "youtube_shorts Shortcode — YouTube Shorts Grid"
...
<ParamField query="batch" type="integer" default="8">
  Number of Shorts fetched per AJAX request when `max="all"`. Accepts `1` through `50`. Defaults to **8** (higher than `youtube_largo`'s default batch to compensate for the denser grid). Ignored when `max` is a fixed number.
</ParamField>
...
| Default `columns` | `3` | `4` |
| Default `batch` | `max(columns × 2, 6)` | `8` |
